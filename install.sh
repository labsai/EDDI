  ...
  if [[ "$WITH_AUTH" == "true" ]]; then
    {
      ...
      # docker-compose.auth.yml has no default for this and refuses to start
      # without it. Single-quoted: compose reads the value literally.
...
repair_running_keycloak() {
  ...
  # install stored. A missing password leaves only the legacy admin/admin to try.
  [[ -n "$KEYCLOAK_ADMIN_USERNAME" ]] || KEYCLOAK_ADMIN_USERNAME=$(env_file_value KEYCLOAK_ADMIN_USERNAME)
  [[ -n "$KEYCLOAK_ADMIN_PASSWORD" ]] || KEYCLOAK_ADMIN_PASSWORD=$(env_file_value KEYCLOAK_ADMIN_PASSWORD)
...
print_success() {
  ...
  if [[ "$WITH_AUTH" == "true" ]]; then
    ...
    # Printed once, here, and stored nowhere: each is a ONE-TIME password that
    # Keycloak replaces at the first login. An account that already had a
    # password was left alone, so a re-run prints nothing new for it.
    local entry account password printed=false
    ...
    if [[ "$DEMO_USERS" != "true" ]]; then
      echo -e "  ${DIM}viewer (read-only) and user (end user) exist without a password; re-run${RESET}"
