...
cleanup() {
  if [[ "$CONTAINERS_STARTED" == "true" && "$HEALTHY" != "true" ]]; then
    ...
    if [[ ${#COMPOSE_FILES[@]} -gt 0 ]]; then
      ...
      docker compose $env_flag ${COMPOSE_FILES[@]/#/-f } down 2>/dev/null || true
    fi
  fi
}
...
resolve_compose_files() {
  ...
  echo "EDDI_VAULT_MASTER_KEY=$EDDI_VAULT_MASTER_KEY" >> "$EDDI_DIR/.eddi-config"
...
case "${1:-help}" in
  start)
    export EDDI_PORT EDDI_HTTPS_PORT EDDI_VAULT_MASTER_KEY
