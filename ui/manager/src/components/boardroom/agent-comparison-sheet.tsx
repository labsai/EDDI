...
function AgentComparisonSheet({ agents, onClose }: AgentComparisonSheetProps) {
  ...
  const metrics: MetricRow[] = [
    {
      label: t("boardroom.comparison.sessions", "Sessions"),
      leftValue: left.sessions,
      rightValue: right.sessions,
      format: formatNum,
    },
    {
      label: t("boardroom.comparison.contributions", "Contributions"),
      leftValue: left.contributions,
      rightValue: right.contributions,
      format: formatNum,
    },
    {
      label: t("boardroom.comparison.errorRate", "Error Rate"),
    ...
    {
      label: t("boardroom.comparison.avgResponse", "Avg Response"),
      leftValue: avgLength(left),
      rightValue: avgLength(right),
      format: formatChars,
    },
    {
      label: t("boardroom.comparison.totalContent", "Total Content"),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t("boardroom.comparison.title", "Agent Comparison")}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border ps-6 pe-4 py-4">
          <h2 className="text-lg font-semibold text-foreground">
            {t("boardroom.comparison.title", "Agent Comparison")}
          </h2>
          <Button
            variant="ghost"
            size="icon"
            onClick={onClose}
            className="h-8 w-8"
            aria-label={t("boardroom.comparison.close", "Close")}
          >
            <X className="h-4 w-4" />
          </Button>
        </div>
