...
function AgentPerformanceSheet({
  ...
  return (
    ...
    <div
      ...
      aria-modal="true"
      aria-label={t(
        "Workforce.analytics.agentPerformance",
        "Agent performance",
      )}
    >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <StatCard
              ...
              label={t("Workforce.analytics.sessions", "Sessions")}
              value={String(agent.sessions)}
            />
            <StatCard
              icon={<MessageSquare className="h-4 w-4" />}
              label={t(
                "Workforce.analytics.contributions",
                "Contributions",
              )}
            ...
            <StatCard
              ...
              label={t("Workforce.analytics.errorRate", "Error Rate")}
              value={`${errorRate.toFixed(1)}%`}
              valueClassName={errorRateColorClass(errorRate)}
            />
            <StatCard
              icon={<Type className="h-4 w-4" />}
              label={t("Workforce.analytics.avgLength", "Avg Length")}
              value={t(
                "Workforce.analytics.avgLengthChars",
                "{{count}} chars",
                { count: avgLength },
              )}
            />
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground mb-3">
              {t(
                "Workforce.analytics.contributionBreakdown",
                "Contribution Breakdown",
              )}
            </h3>
            <ContributionBar
              ...
              errorsLabel={t("Workforce.analytics.errors", "Errors")}
            />
          </div>
        </div>
        ...
        <div className="border-t border-border px-5 py-4 space-y-2">
          {onEditAgent && (
            ...
            >
              ...
              {t("Workforce.analytics.editAgent", "Edit Agent")}
            </Button>
          )}
          {onCompare && (
            <Button
            ...
            >
              ...
              {t("Workforce.analytics.compareAgent", "Compare with…")}
            </Button>
          )}
          <Button variant="ghost" className="w-full" asChild>
            ...
            >
              ...
              {t(
                "Workforce.analytics.chatWithAgent",
                "Chat with Agent",
              )}
            </Link>
          </Button>
