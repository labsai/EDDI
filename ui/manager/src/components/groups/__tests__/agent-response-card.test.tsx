...
describe("AgentResponseCard", () => {
  ...
  it("renders 'No response' for whitespace-only content", () => {
    const wsEntry: TranscriptEntry = {
      ...baseEntry,
      type: "TASK_RESULT",
      content: "   \n  ",
    };
    renderWithProviders(<AgentResponseCard entry={wsEntry} />);
    expect(screen.getByText("No response")).toBeInTheDocument();
  });
  ...
  it("renders emoji verification as structured cards", () => {
    ...
    const { container } = renderWithProviders(
      <AgentResponseCard entry={verifyEntry} discussionStyle="TASK_FORCE" />
    );
    // Structured items render pass/fail subjects
    expect(screen.getByText("Financial Analysis")).toBeInTheDocument();
    expect(screen.getByText("Risk Assessment")).toBeInTheDocument();
    // CheckCircle2 (pass) and XCircle (fail) icons
    expect(container.querySelector("svg.text-emerald-500")).toBeInTheDocument();
    expect(container.querySelector("svg.text-destructive")).toBeInTheDocument();
  });

  it("renders plain text through ReactMarkdown without issues", () => {
    const plainEntry: TranscriptEntry = {
      ...baseEntry,
      content: "Simple response with no special formatting at all",
    };
    renderWithProviders(<AgentResponseCard entry={plainEntry} />);
    expect(
      screen.getByText("Simple response with no special formatting at all")
    ).toBeInTheDocument();
  });
});
