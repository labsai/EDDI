...
import { StepIndicator } from "@/components/boardroom/wizard/step-indicator";
import { TemplatePicker } from "@/components/boardroom/wizard/template-picker";
import {
  TeamBuilder,
  type MemberSlot,
} from "@/components/boardroom/wizard/team-builder";
import {
  ReviewLaunch,
  type CreationProgressItem,
} from "@/components/boardroom/wizard/review-launch";

// ─── Constants ──────────────────────────────────────────────────────────────

const BOARDROOM_PROGRESS_ID = "__boardroom__";
...
function BoardroomWizard() {
  ...
  const steps = useMemo(
    () => [
      { label: t("boardroom.wizard.stepTemplate", "Template") },
      { label: t("boardroom.wizard.stepTeam", "Team") },
      { label: t("boardroom.wizard.stepReview", "Review") },
    ],
    [t],
  );
  ...
  const handleCreate = useCallback(async () => {
    ...
    const progressItems: CreationProgressItem[] = [
      ...members.map((m) => ({
        id: m.id,
        name: m.displayName || t("boardroom.wizard.unnamed", "Unnamed"),
    ...
    for (let i = 0; i < resolvedMembers.length; i++) {
      ...
      try {
        ...
        toast.error(
          t("boardroom.wizard.agentError", "Failed to create {{name}}: {{error}}", {
            name: member.displayName,
            error: message,
          }),
        );
    ...
    updateProgress(BOARDROOM_PROGRESS_ID, { status: "creating" });
    ...
    try {
      ...
      updateProgress(BOARDROOM_PROGRESS_ID, { status: "done" });
