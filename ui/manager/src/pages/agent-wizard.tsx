 ...
 *
 * A `Content-Length` pre-check would short-circuit the common case, but cannot
 * replace this: the header is absent on chunked responses and is not binding
 * even when present.
 */
...
function ApiSpecStep({
  ...
  async function fetchSpec() {
    ...
    try {
      ...
      const text = await readCapped(res, SPEC_MAX_BYTES);
      if (text === null) {
        toast.error(
