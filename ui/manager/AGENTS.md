...
- `updates.ts` holds the only raw `fetch` calls that must **not** carry the auth header: they call `api.github.com` (latest release) and `img.shields.io` (latest published Docker tag), and attaching this deployment's Keycloak token would hand it to a third party. They are also the only calls that are not same-origin, so `ApiClient` could not express them anyway. Docker Hub is read *through* shields.io because `hub.docker.com`, `auth.docker.io` and `registry-1.docker.io` all send no `Access-Control-Allow-Origin` — a direct call is CORS-blocked in production
