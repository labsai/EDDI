...
| **SSE** (Server-Sent Events)                                         | Streaming transport             | Token-by-token chat responses — including tool-enabled turns, which stream over the provider's streaming transport rather than going silent until the tool loop finishes — plus a live `tool_call` event for "Using {tool}…" status, group discussion feeds, and live log streaming |
