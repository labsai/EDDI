...
- **Configuration-driven logic** — bot behavior is JSON, not compiled code
...
> *"Bot behavior belongs in JSON configurations. Java code builds the components that read and execute those configurations."*

**The Principle:** EDDI is a **config-driven engine**, not a monolithic application. The intelligence of a bot — its routing rules, API calls, LLM prompts, output templates — is defined entirely in versioned JSON documents. Java code provides the **infrastructure components** (`ILifecycleTask`, `IResourceStore`, tools) that the engine uses to interpret and execute those configurations at runtime. No bot-specific logic may ever be hardcoded in Java.
...
- Adding `if/else` branches in Java to handle specific bot use cases
...
**EDDI's Answer to "I need custom code":** *"Spin up an external MCP server in an isolated container. EDDI will call it as a tool."* This pushes execution risk outside the EDDI perimeter while providing infinite extensibility.
...
- **Multi-tenancy** — the same engine runs completely different bots for different customers
- **Instant iteration** — changing bot behavior requires editing a JSON document, not a Java rebuild
...
| **MCP** | Opt-in per bot. External MCP tools flagged as READ_ONLY or STATE_CHANGING. STATE_CHANGING requires HITL approval |
| **Tenant Isolation** | Database queries enforce `tenantId` via Java code / SQL WHERE / RLS — never via LLM prompt filtering |
| **Export Sanitization** | Bot ZIP exports scrub API keys, tokens, and high-entropy strings before packaging |
...
> *"Java's concurrency model is our unfair structural advantage."*
...
**Why This Matters:**
- The EU AI Act requires immutable audit logs of *why* an AI made a decision
- Developers suffer from "reasoning blindness" — standard logs show *what* tool was called, not *why* it was chosen
...
- **Business users** who want to configure a bot via visual forms, drag-and-drop pipelines, and guided wizards
...
```
        ...
        Langflow   |         (Only JVM entry)             |   Botpress
                    |                                     |
                    +-------------------------------------+
                    |        Library / Framework           |
                    |                                     |
