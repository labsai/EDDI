...
The distinction matters. Deterministic dependency resolution means nobody can slip a different library into your build; it does **not** mean you can rebuild `eddi-6.4.0.jar` and compare its SHA-256 against ours. Verify published artifacts via the [signature and attestation](#verify-a-published-image), not by rebuilding.
...
```bash
gh attestation verify oci://docker.io/labsai/eddi:6.4.0 --repo labsai/EDDI
```
