...
**Repo:** EDDI (`feat/manager-ingestion-sources`)
...
**Every bound is explicit, because the file is not the operator's data.** 200,000 characters per
document, 500 parts, 5,000 rows × 64 columns per sheet, 64 MB decompressed per archive; DTDs and
external entities off (billion-laughs); an archive naming the same part twice refused outright, since
two entries under one name let two readers disagree about the contents.
...
**Tests**: 33 extraction cases (including a zip bomb, billion-laughs, a duplicate part, an encrypted
PDF, a scan with no text layer, a misnamed spreadsheet and a legacy Office file), a 10-case
`IngestedFileStoreContract` run against in-memory, MongoDB and PostgreSQL, 12 `IngestedFileService`
cases, 11 pipeline cases for the upload path, 13 REST cases, and 6 Manager cases.
