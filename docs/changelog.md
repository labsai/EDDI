...
| Docs | [docs/hitl.md](hitl.md) updated: approver read-scope gate (403 semantics), group approval-status summary/full, pending summaries + `?limit`, group cancel 409, `hitl_resume` SSE event + every-terminal-path-closes guarantee, drift-restores-pause behavior, cancel audit entries, async + projected crash recovery. Known gap: no dedicated group-surface end-to-end IT yet (group HITL is covered by extensive unit/race tests; `HitlPauseResumeIT` exercises the shared machinery end-to-end). |
