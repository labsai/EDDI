...
**Still open.** The §10 question — does a docs-only PR with a skipped `Build & Test` reach `mergeStateStatus: CLEAN`? — remains unobserved: #670 is `APPROVED` but was `DIRTY` on the changelog conflict. Once this merge is pushed, #670 becomes the first clean observation; check it before adding any path-gated job as a required context.
