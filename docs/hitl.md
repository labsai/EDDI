...
|--------|------|-------------|
| `POST` | `/agents/{conversationId}/resume` | Submit a decision (`{"verdict": "APPROVED"\|"REJECTED", "note": "...", "pauseId": "..."}` — verdict is case-insensitive). `pauseId` is optional: when set (from `approval-status`), the decision applies only while the conversation is still in that pause; if it has since been resumed and paused again on a different request the decision is refused with `409` and the current pause is left untouched. The group approve body accepts the same field inside its `decision`, and the MCP `resume_conversation` tool takes it as an optional argument (`errorCode: PAUSE_CHANGED`) |
