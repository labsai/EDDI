...
|--------|------|-------------|
| `POST` | `/agents/{conversationId}/resume` | Submit a decision (`{"verdict": "APPROVED"\|"REJECTED", "note": "...", "pauseId": "..."}` — verdict is case-insensitive). `pauseId` is optional: when set (from `approval-status`), the decision applies only while the conversation is still in that pause; if it has since been resumed and paused again on a different request the decision is refused with `409` and the current pause is left untouched. The group approve body accepts the same field inside its `decision`, and the MCP `resume_conversation` and `approve_group_phase` tools take it as an optional argument (`pauseId` from `get_approval_status` / `get_group_approval_status`; a changed pause answers `errorCode: PAUSE_CHANGED`) |
| `GET` | `/agents/{conversationId}/approval-status` | Pause summary: `state`, `pausedAt`, `pauseId` (pass it back as the decision's `pauseId` — see below), `pauseReason`, `timeoutPolicy`, `approvalTimeout` (render the auto-decision deadline as `pausedAt + approvalTimeout`), `pauseDetails` — structured, computed at read time (see below). `?detail=full` for the whole snapshot — approver-only callers get `full` only while the conversation is paused, `403` otherwise |
...
**Approver read scope:** the approver role exists to *decide* pending approvals, not as a universal read grant. Approver-only callers (not owner, not admin) may read `approval-status?detail=full` — the full memory snapshot / transcript — only while the conversation is actually awaiting approval; outside a pause they get `403` and can use the summary view. The full snapshot is also **sanitized** for every caller, on REST and on the MCP `get_approval_status` tool alike: the pending batch's `argumentsRaw`, frozen LLM transcript (`chatTranscriptJson`), running trace (`traceSoFar`) and batch-level `fingerprint` (an unsalted digest over the raw arguments) are removed, the per-call request fingerprint is replaced by a constant marker, and `argumentsRedacted` and the request preview are re-redacted with the current filter. Both surfaces call the same method (`ConversationMemoryUtilities.sanitizePendingToolCallsForApprover`), so they cannot serve different fields.

**Whose credentials a resumed turn carries.** When the approver *is* the conversation's owner, the resumed turn runs as them, exactly like a `say`. When somebody else decides — an admin or an `eddi-approver` — their identity covers only what they were shown: each tool call they explicitly approved in a `TOOL_CALL` pause runs with their caller identity (so a `${caller:token}` header on that call carries the approver's token), and the rest of the resumed turn runs with **no caller**. A `${caller:token}` / `${caller:userId}` reference elsewhere in that turn — including every call after a `RULE` pause, which previews nothing — therefore fails closed, as it does on a scheduled turn, instead of going out with the approver's credentials. A resumed group discussion follows the same rule: it runs as the approver only when the approver started the discussion. `PER_USER` connection credentials always resolve against the conversation's owner, never the approver.
...
- **Operator self-URL** (`GET /administration/operator/self-url`, `eddi-admin`): answers the address this deployment can reach *itself* at, which is what the operator's generated tools must target. `eddi.self.base-url` when set, otherwise `http://127.0.0.1:${quarkus.http.port}`; the response is `{baseUrl, source}` with `source` one of `configured`, `loopback` or `unresolved`. `unresolved` (with `baseUrl: null`) means a random HTTP port (`quarkus.http.port=0`) and no override — the Manager then refuses to guess and asks for the base URL explicitly. It exists because the only other candidate a provisioning client has is its own browser origin, and the two coincide only when nothing sits between the browser and EDDI — a tunnelled staging instance was provisioned with `localhost:7080` in front of a container on `:7070`, and the operator reported itself deployed and gate-verified while all 22 of its tools failed on connect. Deliberately a GET with no arguments: the answer must not depend on a `Host` header or an `X-Forwarded-*` chain a proxy can rewrite. Note that pointing the tools at that address makes them cross-origin to `${caller:token}`'s same-origin rule, which is why `CallerIdentityResolver` also releases the token when the target is this very process — the same argument `LoopbackCallerAuthFilter` makes for the internal hop; the value comes from deployment configuration alone. It is not narrower than same-origin: the self address bypasses whatever sits in front of EDDI, so what the token reaches is what EDDI's own authorization allows. A deployment that also relies on its proxy's rules sets `eddi.caller-identity.self-release.enabled=false`; a caller whose origin could not be captured never gets the self release.
...
| `hitlApproverUserIds` | Comma-separated Slack user ids allowed to decide via buttons; an entry `T…:U…` matches only that user of that team. **Without this list, buttons are not rendered and interactive decisions are rejected — fail-closed.** |
...
- **A decision is bound to its integration, its conversation and its pause.** The button value is `<integrationName>|<subject>|<pauseId>`. The request is verified with *that* integration's signing secret only; the targeted conversation must have been started by that integration — its `channelIntegrationId` start context holds the integration's store resource id, which unlike the name cannot be renamed onto or reused by another integration — and its agent must be one of the integration's targets; a group discussion must have been started through that integration (recorded when Slack starts it) and its group must be one of its targets. Conversations and discussions started before this binding existed cannot be decided from Slack: their cards are posted without buttons and they stay decidable from the Manager and the API. `hitlApproverUserIds` entries may be team-scoped (`T…:U…`, matching only that user of that team), and when the integration pins `teamId` a clicking user whose `user.team_id` differs (a Slack Connect user) is refused even if their bare id is listed. `decidedBy` stays `slack:<userId>`. The `pauseId` must still be the conversation's current pause: a card left in the channel after the conversation was resumed and paused again is marked "out of date" and cannot approve the newer request. A card without a pause id (posted by an older release) is refused the same way — decide those pauses from the Manager or the API. When the pause could not be identified at posting time the card is rendered without buttons. Integration names must be unique among routed integrations of a type and must not contain `|`.
...
| `approve_group_phase`             | `POST /groups/{groupId}/conversations/{gcId}/approve`        | Optional `taskApprovals` JSON for TASK granularity and optional `pauseId` (from `get_group_approval_status`; a changed pause answers `PAUSE_CHANGED`); blocks and returns the resumed discussion (no SSE variant over MCP) |
