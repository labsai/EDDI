...
- **B6: the in-place guard could be evaded.** It counted only dated `## … (YYYY-MM-DD` headings, so an undated heading or a setext (`---`-underlined) heading went through. The whole job was also skipped for any head branch *named* `chore/collate-changelog`, a fork's included.
  ...
  - Like the collator, it skips lines inside fenced code blocks: a quoted `## Example (date)` or a YAML `---` in an example is not an entry. The awk follows fences through each side's lines and the diff context. A fence opened above a hunk's first context line is invisible to it; with git's three lines of context that is rare, and the job is not a required check.
...
- **B5: Node was floating and never verified.** Every `setup-node` said `22`, while `pom.xml` vendors `v22.23.2` for the shipped UI build. frontend-maven-plugin 1.15.1 downloads Node without any checksum and reuses whatever archive sits in the local Maven repository, which setup-java restores from cache.
  ...
  - A local `./mvnw package` still trusts nodejs.org over TLS, as documented in `pom.xml`. So does `src/main/docker/Dockerfile.demo`, which runs `mvn package` without `-DskipUi`. It is not the shipped image, and fix-plan item 16 (B1) is reworking that file, so it is documented here rather than changed.
- **B4: `.gitignore` re-included nested `.claude/` directories.** `!.claude/` was unanchored, while `.claude/*` is root-only, so `ui/chat/.claude/settings.local.json` was not ignored. The three root rules are now anchored with `/`. The Manager's `ui/manager/.claude/` had the same shape: the whole directory was re-included, so a `worktrees/` checkout created there was addable. It is now re-excluded apart from `skills/` and `launch.json`. `nestedClaudeDirectoriesStayIgnored` asks `git check-ignore` directly. `.gitignore` joins the `backend` path filter so a change to it runs that test.
...
- **L7: README dev commands lacked the Jlama flag.** Every `quarkus:dev` command in README, AGENTS.md, CONTRIBUTING.md, `docs/getting-started.md` and `docs/developer-quickstart.md` now passes `-Djvm.args=--add-modules=jdk.incubator.vector`, as `mise run dev` does, with a note that only `jlama` agents need it. `JlamaRuntimeFlagsTest.documentedDevCommandsPassTheFlag` checks all of them except CONTRIBUTING.md, which is in no `backend` path filter, so `BuildQualityGatesTest` forbids a test reading it.
...
- AGENTS.md §2 rule 8 and `docs/changelog.d/README.md` describe the new guard and the `--check` step.
- The Node-pin test requires every workflow that uses `env.NODE_VERSION` to declare it itself, at the pom's value. An undeclared one would silently be empty.
