...
- **High 7 + S1 (frontend) — silent secret overwrite and grant widening.** Rotate POSTed to a `/rotate`
  ...
  key deleted in the meantime is refused instead of re-created. The re-read and the existence check use
  the exact per-key metadata read, `GET /secretstore/secrets/{tenant}/{key}` (404 = absent, anything else
  is raised, never read as "free to create"). "Add Secret" on the secrets page, the
...
- **High 8 — the approval queue could approve a pause nobody saw.** Queue rows (rule, group) and the
  ...
  refusing on any mismatch — and refusing outright a shown pause that carries neither an id nor a start
  instant. The group approve answers every wrong-state refusal (a #839 pause mismatch included) with
  "Group conversation is not awaiting approval…"; that 409 is read as a changed pause too, so the queue
  says "this request changed since you opened it" and refreshes the group queue.
...
- **Share dialog.** A visibility click cascaded over the whole config graph immediately, replacing each
  ...
  failed apply keeps the panel open with the reason.
- **Channels.** Bot token and signing secret are reference-only (`${vault:…}`; `${vars:…}` is refused
  ...
  reference-only picker until typed over (re-masked when another channel is loaded into the page),
  redacted in the Raw Configuration panel, and flagged by a banner. The detail draft is seeded once per
  id and version, so a refetch returning changed data no longer overwrites unsaved edits.
...
- **Lost-key flow (#836) — moved out.** An "Adopt master key" step for the secrets danger zone was
...
- **"Every channel's full config, tokens included, is fetched into the query cache."** Partly wrong: the
  ...
  of those fields and there is no projection endpoint. It carries no plaintext token for channels saved
  through the Manager since this change; a legacy channel stored with one still sends it on that read
  until it is re-saved (its detail page now says so).
