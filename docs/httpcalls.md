...
| httpCall.preRequest.batchRequests.maxBatchSize                              | (`Integer`, default `100`, at most `1000`) most requests the batch may expand into. A target array with more elements refuses the whole call — nothing is sent and the turn fails with a message naming the limit — instead of silently dropping the tail. |
