...
rather than being stored. Driven by a customer deployment: the integrating backend calls EDDI as one service
...
- **`${caller:token}`** is same-origin only, and deliberately so. It relays EDDI's *own* credential
  back to the origin the caller addressed. The integrating service is a different origin *and* a different credential.
...
authenticate to EDDI directly: the row is keyed by principal, and that deployment's topology yields a
...
**Round 2 — CodeRabbit review on #670 (8 comments, all addressed).** Six were real defects in the CI design, two were nitpicks. The valuable ones: (1) matrixing the `ci.yml` CodeQL job to add TypeScript **renames its check** from `CodeQL Analysis` to `CodeQL Analysis (java-kotlin)` — and `CodeQL Analysis` is one of only two required contexts on `main`, so the naive edit wedges every PR on a status that never reports; it also collides with `codeql.yml`'s identically-named job, so both need distinct names and the protection update must land in the same change. (2) Adding `ui-build-and-test` to `docker.needs` would **stop publishing images on backend-only pushes**, because GitHub skips a dependent job when any dependency is skipped and the UI job is path-gated — fixed with an always-running `ui-gate` aggregator that demands UI success only when `outputs.ui == 'true'`. (3) `matrix` is **not available in a job-level `if:`** (GitHub evaluates it before matrix expansion), so the "skip the postgres leg on PRs" condition would not have worked — replaced with a `fromJSON` matrix emitted by `detect-changes`. (4) The imported chat `vite.config.ts` still points `outDir` at `../EDDI/...`, which from `ui/chat` resolves to a non-existent `ui/EDDI` — so the Phase-1 "it compiles" checkpoint was a lie; the retarget moved into the import commit. (5) `.gitignore` does not stop Maven's **default** resource copy, so a developer upgrading from a pre-monorepo checkout would stage 562 stale generated files into `target/classes`, and `copy-resources` never prunes — fixed with excludes on the default `<resources>` entry plus a dirty-workspace test. (6) Phase 0 called Gitleaks/Trivy/dependency-review "blocking" when none is a required context and `docker` is push-only, so a PR can merge with a failed scan and only fail after merge. Nitpicks: one MD040 unlabeled fence, and a `docs/planning/` path reference already fixed in the previous commit.
...
**Revision 2 (same day):** two adversarial review passes with local dry-runs, all findings folded into the plan. The multi-page Vite build was **executed for real** in the Manager checkout (works: three shells share one `main-<hash>.js` entry, `__auth_config__.js` and app-loader preserved, no stray `index.html`; reverted after). New hard findings: (1) the CI job split needs explicit output re-export plumbing or `release`/`smoke-test`/`preflight-push` break; (2) the migration PR must merge with a **merge commit** — the AGENTS.md squash guidance would flatten the imported histories; (3) gitleaks scans all ~1,079 imported commits and reads `.gitleaksignore` from the *base* branch, so any ignore entries must land on main first; (4) `dependency-review` (`fail-on-severity: high`, GPL/AGPL deny) sees the full npm graph at once — pre-check required; (5) Manager's `public/mockServiceWorker.js` would ship in the production jar via the dist copy — now excluded, with 404 asserted in E2E; (6) `ui/node/` (frontend-maven-plugin's vendored Node) wasn't gitignored; (7) every `mvnw compile/test/verify/quarkus:dev` triggers the npm build unless `-DskipUi=true` — wired into build-and-test, integration-test, and both CodeQL workflows; (8) `eddi-chat-ui` tracks files under `dist/` (ignore-overridden), and its committed bundle already differs byte-wise from a fresh same-hash build — drift proof; (9) statics are now single-sourced (icons from Manager `public/`, fonts/img from chat `public/`, redirect shell + robots + landing-redirect stay backend; unreferenced `loading-indicator.svg` deleted); (10) E2E only exercises the Vite dev server, so a shipped-shell curl gate was added. Plan now carries a verified-facts table (V1–V18), full CI rewiring spec, and a post-migration reconfiguration checklist (branch-protection contexts, local tooling, external links).
