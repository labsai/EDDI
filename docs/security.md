...
- **Behaviour:** With `enforceBudget: true`, the budget is checked *before* each call using `<=` — the call that crosses the ceiling completes and the next one returns `Error: Budget exceeded for conversation <id>` to the LLM. Without `enforceBudget` the cost is tracked and reported but no call is ever refused: the ceiling was inert for its whole shipped life (every built-in priced at $0.00), so enforcing it automatically would newly break agents whose stored config merely carries the number
