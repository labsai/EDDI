...
- **Configuration:** `enableCostTracking` (default `true`), `toolPricing` (per-call price overrides), `maxBudgetPerConversation` (no default — unlimited), `enforceBudget` (default `true`, deployment fallback `eddi.tools.budget.enforce-by-default`)
...
- **Behaviour:** A configured `maxBudgetPerConversation` is enforced by default. The budget is checked *before* each call using `<=` — the call that crosses the ceiling completes and the next one returns `Error: Budget exceeded for conversation <id>` to the LLM. `enforceBudget: false` (per task) or `eddi.tools.budget.enforce-by-default=false` (per deployment) makes the ceiling report-only. Enforcement is not opt-in, because only *built-ins* were exempt from it before prices resolved: http/MCP/A2A/dynamic tools dispatch under their configured name, so an agent with a tool called `websearch`/`webscraper`/`pdfreader` was already having calls refused, and defaulting the new flag to `false` would have deleted that ceiling on upgrade
