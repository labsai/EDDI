...
> the instance's cloud credentials; nobody configures it as an API. One caveat
> remains: with protection off redirects are still followed, so only turning
> protection on stops a public URL that redirects to it.
