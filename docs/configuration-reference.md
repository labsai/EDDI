...
| `eddi.http.limits.default-max-body-size` | `25M` | Largest request body any endpoint accepts, **except** a knowledge base's file upload (`POST /ragstore/rags/{id}/sources/{sourceId}/files`), which keeps `quarkus.http.limits.max-body-size` (60 MB). That global limit was raised for the upload and so raised it for every endpoint; this holds the rest to their old ceiling. Judged on the declared `Content-Length`, before the body is read — a chunked request is bounded by the global limit only. See [rag.md](rag.md#ingestion-sources) |
