...
| `eddi.tools.pdf-reader.max-download-bytes` | `26214400` (25 MB) | Largest PDF the PDF-reader tool downloads, enforced the same way. The download is held in memory (peak about twice the limit); no temp file is written, and it gets a 120 s budget |
