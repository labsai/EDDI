...
public class OutputItemContainerGenerationTaskTest {
        ...
        public void executeTask() {
                ...
                when(currentStep.getLatestData(eq(ACTIONS))).thenAnswer(
                                invocation -> new Data<>(ACTION_1, asList(SOME_ACTION_1, SOME_OTHER_ACTION_1)));
                ...
                when(conversationStep.getLatestData(eq(ACTIONS)))
                                .then(invocation -> new Data<>(ACTION_2, asList(SOME_ACTION_2, SOME_OTHER_ACTION_2)));

                var expectedOutputData = new Data<>(OUTPUT_TEXT + ACTION_1, new TextOutputItem(ANSWER_ALTERNATIVE_1),
                                asList(new TextOutputItem(ANSWER_ALTERNATIVE_1),
                                                new TextOutputItem(ANSWER_ALTERNATIVE_2)));

                when(dataFactory.createData(eq(OUTPUT_TEXT + ACTION_1), any(TextOutputItem.class),
                                eq(asList(new TextOutputItem(ANSWER_ALTERNATIVE_1),
                                                new TextOutputItem(ANSWER_ALTERNATIVE_2)))))
