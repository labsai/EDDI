...
class PropertySetterTaskTest {
    ...
    class ExecuteTests {
        ...
        @Test
        @DisplayName("CATCH_ANY_INPUT_AS_PROPERTY with empty input — should skip gracefully")
        void catchAnyInput_emptyInput() throws Exception {
            var memory = mock(IConversationMemory.class);
            var currentStep = mock(IWritableConversationStep.class);
            when(memory.getCurrentStep()).thenReturn(currentStep);
            ...
            // input:initial exists but result is empty string
            var inputData = mock(IData.class);
            when(currentStep.getLatestData("input:initial")).thenReturn(inputData);
            when(inputData.getResult()).thenReturn("");
