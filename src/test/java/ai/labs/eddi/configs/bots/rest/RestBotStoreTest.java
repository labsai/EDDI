...
class RestAgentStoreTest {
    ...
    @Mock
    private IRestPipelineStore restPipelineStore;
    @Mock
    ...
    void setUp() {
        ...
        restAgentStore = new RestAgentStore(
                AgentStore, restPipelineStore, documentDescriptorStore, jsonSchemaCreator, scheduleStore);
    }
    ...
    class DeleteBotTests {
        ...
        void deleteBot_noCascade() throws Exception {
            ...
            verify(restPipelineStore, never()).deletePackage(anyString(), anyInt(), anyBoolean(), anyBoolean());
        ...
        void deleteBot_cascade() throws Exception {
            ...
            config.setPipelines(new ArrayList<>(List.of(
                    URI.create("eddi://ai.labs.package/PipelineStore/packages/" + PKG1_ID + "?version=2"),
                    URI.create("eddi://ai.labs.package/PipelineStore/packages/" + PKG2_ID + "?version=1")
            )));
            ...
            when(AgentStore.getBotDescriptorsContainingPackage(PKG2_ID, 1, false))
                    .thenReturn(List.of(dummyDescriptor()));
            when(restPipelineStore.deletePackage(anyString(), anyInt(), anyBoolean(), anyBoolean()))
                    .thenReturn(Response.ok().build());

            restAgentStore.deleteBot(AGENT_ID, 1, true, true);

            verify(restPipelineStore).deletePackage(PKG1_ID, 2, true, true);
            verify(restPipelineStore).deletePackage(PKG2_ID, 1, true, true);
        ...
        void deleteBot_cascade_skipsSharedPackages() throws Exception {
            ...
            verify(restPipelineStore, never()).deletePackage(eq(PKG1_ID), anyInt(), anyBoolean(), anyBoolean());
            verify(restPipelineStore).deletePackage(PKG2_ID, 1, true, true);
            verify(AgentStore).deleteAllPermanently(AGENT_ID);
        }
        ...
        void deleteBot_cascade_partialFailure() throws Exception {
            ...
            when(AgentStore.getBotDescriptorsContainingPackage(anyString(), anyInt(), eq(false)))
                    .thenReturn(List.of(dummyDescriptor()));
            when(restPipelineStore.deletePackage(PKG1_ID, 1, true, true))
                    .thenThrow(new RuntimeException("Package in use"));
            when(restPipelineStore.deletePackage(PKG2_ID, 1, true, true))
                    .thenReturn(Response.ok().build());

            assertDoesNotThrow(() -> restAgentStore.deleteBot(AGENT_ID, 1, true, true));

            verify(restPipelineStore).deletePackage(PKG1_ID, 1, true, true);
            verify(restPipelineStore).deletePackage(PKG2_ID, 1, true, true);
            verify(AgentStore).deleteAllPermanently(AGENT_ID);
        }
        ...
        void deleteBot_cascade_emptyPackages() throws Exception {
            AgentConfiguration config = new AgentConfiguration();
            config.setPipelines(new ArrayList<>());
            when(AgentStore.read(AGENT_ID, 1)).thenReturn(config);
