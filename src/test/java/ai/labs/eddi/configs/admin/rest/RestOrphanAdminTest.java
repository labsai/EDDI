...
class RestOrphanAdminTest {
    ...
    class ScanOrphansTests {
        ...
        void scanOrphans_noOrphans() throws Exception {
            ...
            when(documentDescriptorStore.readDescriptors(eq("ai.labs.agent"), eq(""), anyInt(), anyInt(), eq(false))).thenReturn(List.of(agentDesc))
                    .thenReturn(Collections.emptyList());

            AgentConfiguration agentConfig = new AgentConfiguration();
            agentConfig
                    .setWorkflows(new ArrayList<>(List.of(URI.create("eddi://ai.labs.workflow/workflowstore/workflows/" + PKG1_ID + "?version=1"))));
            ...
            ext.setConfig(new HashMap<>(Map.of("uri", "eddi://ai.labs.rules/rulestore/rulesets/" + BEH1_ID + "?version=1")));
    ...
    class PurgeOrphansTests {
        ...
        void purgeOrphans_deletesOrphans() throws Exception {
            ...
            DocumentDescriptor agentDesc = descriptor("ai.labs.agent", AGENT1_ID, 1, "TestAgent");
            when(documentDescriptorStore.readDescriptors(eq("ai.labs.agent"), eq(""), anyInt(), anyInt(), eq(true))).thenReturn(List.of(agentDesc))
                    .thenReturn(Collections.emptyList());
            ...
            for (String type : List.of("ai.labs.rules", "ai.labs.apicalls", "ai.labs.output", "ai.labs.llm", "ai.labs.property", "ai.labs.dictionary",
                    "ai.labs.parser")) {
                when(documentDescriptorStore.readDescriptors(eq(type), eq(""), anyInt(), anyInt(), eq(true))).thenReturn(Collections.emptyList());
            }

            when(resourceClientLibrary.deleteResource(any(), eq(true))).thenReturn(Response.ok().build());
    ...
    /**
     * Helper to set up mock returns for all store types. Unspecified types return
     * empty lists.
     */
    private void setupStoreReturns(Map<String, List<DocumentDescriptor>> storeDescriptors) throws Exception {
        for (String type : List.of("ai.labs.package", "ai.labs.rules", "ai.labs.apicalls", "ai.labs.output", "ai.labs.llm", "ai.labs.property",
            ...
            // boundary)
            when(documentDescriptorStore.readDescriptors(eq(type), eq(""), eq(0), anyInt(), eq(false))).thenReturn(descs);
            if (!descs.isEmpty()) {
