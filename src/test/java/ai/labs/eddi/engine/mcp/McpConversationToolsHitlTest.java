...
class McpConversationToolsHitlTest {
    ...
    @Test
    void talkToAgent_toolCallPause_envelopeCarriesPauseTypeAndToolNames() throws Exception {
        ...
        assertTrue(result.contains("PAUSED_FOR_APPROVAL"), result);
        assertTrue(result.contains("\"pauseType\":\"TOOL_CALL\""), result);
        assertTrue(result.contains("delete_production_db"), result);
        assertTrue(result.contains("wire_transfer"), result);
        // Existing fields must be preserved.
        assertTrue(result.contains("\"suggestNextTool\":\"resume_conversation\""), result);
        assertTrue(result.contains("AWAITING_HUMAN"), result);
        // Privacy: NEVER leak arguments (raw or redacted).
        assertFalse(result.contains("topsecret"), result);
        assertFalse(result.contains("token"), result);
    }

    @Test
    void talkToAgent_rulePause_envelopeUnchanged() throws Exception {
        // Task 13 regression: a RULE pause envelope must be unchanged — no pauseType
        // and no tools fields, but all previously-present fields intact.
        var snapshot = new SimpleConversationMemorySnapshot();
        snapshot.setConversationState(ConversationState.AWAITING_HUMAN);
        ...
        assertTrue(result.contains("\"suggestNextTool\":\"resume_conversation\""), result);
        assertTrue(result.contains("AWAITING_HUMAN"), result);
        assertFalse(result.contains("pauseType"), result);
        assertFalse(result.contains("\"tools\""), result);
    }
