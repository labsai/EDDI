 ...
 * <p>
 * Ported from {@code RestAgentEngineTest} in EDDI-integration-tests. Uses
 * Quarkus DevServices for MongoDB (requires Docker).
 * <p>
...
public class AgentEngineIT extends BaseIntegrationIT {
    ...
    void setUp() throws Exception {
        if (!agentsDeployed) {
            agentResourceId = setupAndDeployAgent("agentengine/dictionary.json", "agentengine/rules.json", "agentengine/output.json");
            agent2ResourceId = setupAndDeployAgent("agentengine/dictionary2.json", "agentengine/rules2.json", "agentengine/output2.json");
            agentsDeployed = true;
        }
    ...
    void checkWelcomeMessage() throws Exception {
        ...
        response.then().assertThat().statusCode(200).body("agentId", equalTo(agentResourceId.id()))
                ...
                .body("environment", equalTo("production")).body("conversationState", equalTo("READY")).body("undoAvailable", equalTo(false))
    ...
    void checkWordInputSimpleConversationLog() {
        Response response = sendUserInput(agentResourceId.id(), conversationResourceId.id(), "hello", false, false);
        ...
        response.then().assertThat().statusCode(200).body("conversationSteps", hasSize(2))
                ...
                .body("undoAvailable", equalTo(true)).body("redoAvailable", equalTo(false));
    }

    @Test
    @DisplayName("should return only current step when returnCurrentStepOnly=true")
    void checkWordInputCurrentStepOnly() {
        Response response = sendUserInput(agentResourceId.id(), conversationResourceId.id(), "hello", false, true);

        response.then().assertThat().statusCode(200).body("conversationSteps", hasSize(1))
    ...
    void checkSecondTimeWordInput() {
        ...
        response.then().assertThat().statusCode(200).body("conversationSteps", hasSize(3))
    ...
    void checkSecondAgentDeployed() {
        ...
        response.then().assertThat().statusCode(200).body("agentId", equalTo(agent2ResourceId.id())).body("conversationSteps", hasSize(2))
    ...
    void testStringContextSendWithInput() {
        ...
        response.then().assertThat().statusCode(200).body("conversationSteps[1].conversationStep[0].key", equalTo("context:someContextKeyString"))
    ...
    private ResourceId setupAndDeployAgent(String dictionaryPath, String behaviorPath, String outputPath) throws Exception {
        String dictionary = load(dictionaryPath);
        String behavior = load(behaviorPath);
        String output = load(outputPath);

        String locationDictionary = createAgentResource(dictionary, "/dictionarystore/dictionaries");
