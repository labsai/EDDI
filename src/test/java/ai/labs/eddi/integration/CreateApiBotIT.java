...
/**
 * Integration test for the create_api_bot pipeline, testing against a <b>running EDDI instance</b>.
 * <p>
 * This is a standalone JUnit test (not {@code @QuarkusTest}) that exercises the full
 * REST API. It validates the resource creation pipeline that {@code create_api_bot} produces:
 * <ol>
 *   <li>Create 2 HttpCallsConfiguration resources (grouped by tag: users, orders)</li>
 ...
 * The {@code quarkus-mcp-server-http} extension's build-time deployment processor generates
 * CDI injection points from {@code @ToolArg} parameters during test augmentation, causing
 * {@code UnsatisfiedResolutionException}. This is an MCP extension limitation — production
 * builds work fine.
 * <p>
 * <b>Running:</b><br>
 * Start EDDI first, then run:
 * <pre>
 *   mvn verify -Dit.test=CreateApiBotIT -Deddi.base-url=http://localhost:7070
 * </pre>
 * Default base URL: {@code http://localhost:7070}
 */
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
@Tag("running-instance")
public class CreateApiBotIT {
    ...
    void readHttpCallsUsers() {
        ...
        given()
                ...
                .body("httpCalls[0].name", equalTo("listUsers"))
                .body("httpCalls[0].description", equalTo("List all users"))
                .body("httpCalls[0].actions[0]", equalTo("api_get_users"))
                .body("httpCalls[0].request.path", equalTo("/users"))
                .body("httpCalls[0].request.queryParams.limit", equalTo("[[${limit}]]"))
                .body("httpCalls[1].name", equalTo("getUser"))
    ...
    void createLangchain() {
        ...
        given()
                ...
                .body("tasks[0].parameters.systemMessage",
                        containsString("Available API endpoints"));
    }
