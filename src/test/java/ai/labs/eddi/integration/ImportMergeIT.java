 ...
 * re-import (merge),
 * verifying that merge imports update existing resources instead of creating
 * duplicates.
 */
@QuarkusTest
...
public class ImportMergeIT extends BaseIntegrationIT {
        ...
        void importCreate() throws Exception {
                ...
                Response descriptorResponse = given()
                                .get("/descriptorstore/descriptors/" + firstImportAgentId.id() +
                                                "?version=" + firstImportAgentId.version());
        ...
        void exportAgent() {
                ...
                Response exportResponse = given()
                                .post("/backup/export/" + firstImportAgentId.id() +
                                                "?agentVersion=" + firstImportAgentId.version());
        ...
        void previewMergeImport() {
                ...
                boolean agentIsUpdate = preview.resources().stream()
                                .anyMatch(r -> "agent".equals(r.resourceType()) &&
        ...
        void mergeImportReusesId() {
                ...
                assertThat("Merge should reuse the same Agent ID",
                                mergedAgentId.id(), equalTo(firstImportAgentId.id()));

                // The version should be incremented (updated, not duplicated)
                assertThat("Merge should create a new version",
                                mergedAgentId.version(), greaterThan(firstImportAgentId.version()));
        }
        ...
        void selectiveMerge() {
                ...
                String agentOriginId = preview.resources().stream()
                                .filter(r -> "agent".equals(r.resourceType()))
                ...
                Response selectiveResponse = given()
                                .contentType("application/zip")
                                .queryParam("strategy", "merge")
                                .queryParam("selectedResources", agentOriginId)
                                .body(exportedZipBytes)
                                .post("/backup/import");
        ...
        void createAlwaysNew() throws Exception {
                ...
                assertThat("Create strategy should produce a new Agent ID (not merge)",
                                secondAgentId.id(), not(equalTo(firstImportAgentId.id())));
        }
