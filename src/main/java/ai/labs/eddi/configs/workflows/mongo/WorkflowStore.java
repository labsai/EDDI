...
public class WorkflowStore extends AbstractResourceStore<WorkflowConfiguration> implements IWorkflowStore {
        ...
        @Override
        public IResourceStore.IResourceId create(WorkflowConfiguration WorkflowConfiguration)
                        throws IResourceStore.ResourceStoreException {
                RuntimeUtilities.checkCollectionNoNullElements(WorkflowConfiguration.getWorkflowSteps(),
                                WORKFLOW_EXTENSIONS_FIELD);
        ...
        @Override
        public List<DocumentDescriptor> getWorkflowDescriptorsContainingResource(String resourceURI,
                ...
                do {
                        ...
                        allIds.addAll(resourceStorage.findResourceIdsContaining(
                                        WORKFLOW_EXTENSIONS_CONFIG_URI_FIELD, resourceURI));
                        allIds.addAll(resourceStorage.findHistoryResourceIdsContaining(
                                        WORKFLOW_EXTENSIONS_CONFIG_URI_FIELD, resourceURI));

                        // Search in dictionaries config.uri field
                        allIds.addAll(resourceStorage.findResourceIdsContaining(
                                        WORKFLOW_EXTENSIONS_DICTIONARIES_CONFIG_URI_FIELD, resourceURI));
                        allIds.addAll(resourceStorage.findHistoryResourceIdsContaining(
                                        WORKFLOW_EXTENSIONS_DICTIONARIES_CONFIG_URI_FIELD, resourceURI));

                        // Sort and deduplicate
                        Comparator<IResourceStore.IResourceId> comparator = Comparator
                                        .comparing(IResourceStore.IResourceId::getId)
                        ...
                        for (IResourceStore.IResourceId workflowId : allIds) {
                                ...
                                boolean alreadyContainsResource = ret.stream().anyMatch(
                                                resource -> {
                                                        var id = RestUtilities.extractResourceId(resource.getResource())
                                                                        .getId();
                                ...
                                try {
                                        var packageDescriptor = documentDescriptorStore.readDescriptor(
                                                        workflowId.getId(), workflowId.getVersion());
