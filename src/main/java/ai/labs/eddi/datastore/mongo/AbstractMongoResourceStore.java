 ...
 * <p>
 * Extends the database-agnostic {@link AbstractResourceStore} and provides
 * a constructor that accepts {@link MongoDatabase} directly for backward
 * compatibility with existing stores during the migration to the factory pattern.
 * <p>
...
public abstract class AbstractMongoResourceStore<T> extends AbstractResourceStore<T> {
     ...
     * internally.
     *
     * @deprecated Use {@link AbstractResourceStore} with IResourceStorageFactory instead
     */
    @Deprecated
     ...
     * instances.
     * Used by AgentStore and WorkflowStore which have inner classes extending
    ...
    private static <T> HistorizedResourceStore<T> createHistorizedStore(
            MongoDatabase database,
            String collectionName,
            IDocumentBuilder documentBuilder,
            Class<T> documentType) {
        RuntimeUtilities.checkNotNull(database, "database");
        MongoResourceStorage<T> resourceStorage = new MongoResourceStorage<>(
                database, collectionName, documentBuilder, documentType);
