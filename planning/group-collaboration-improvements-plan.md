...
- **Signing:** Ed25519 per transcript entry when `security.signInterAgentMessages=true` (inside `executeAgentTurn`); receiver verification incremental via the `lastVerifiedIndex` class-field map; nonce replay protection via `NonceCacheService`.
