...
| F5 | **`div.kc-logo-text` is absent from `keycloak.v2`'s template — but a realm can inject it.** *(Corrected after the admin console work.)* The rule lives in `styles.css` while the element is not in `template.ftl`, so it is dead **only for a realm whose `displayNameHtml` is plain text**. Keycloak's own `master` realm ships `displayNameHtml` as `<div class="kc-logo-text"><span>Keycloak</span></div>`, which renders that div inside `#kc-header-wrapper` and paints the Keycloak logo on top of ours | Targeting `div.kc-logo-text` for the logo still renders nothing on the EDDI realm, which is why the wordmark goes on `#kc-header-wrapper`. But the theme must also suppress any logo box a realm injects there | `curl` the template and grep for `kc-logo-text` → no match; then `GET /admin/realms/master` and read `displayNameHtml` |
