...
- **the HTTP method and path** — reconstructed client-side by mapping `toolName → WRITE_ENDPOINTS` entry via the fetched spec's `operationId`. Without it the admin approves `patchDescriptor({name:"x"})` with no idea *what* is being patched or by which verb.

  **This reconstruction is not sufficient on its own, and must not be treated as if it were.** It is a client-side guess from a spec that can drift; templating and body resolution still happen server-side *after* approval, so the request executed can differ from the one shown. Closing that properly needs a **server-side redacted preview plus an immutable fingerprint of the resolved request, re-checked immediately before execution** — approve the fingerprint, not the tool name. That is backend work beyond this PR, so it gates step 5: **until the fingerprint check exists, `WRITE_ENDPOINTS` stays empty** and everything else here ships as a read-only improvement. Labelling the display "reconstructed" is honesty about the gap, not a mitigation of it.
...
**After any decision.** `resumeConversation` returns an empty body and the continuation runs async (`ConversationService.java:1577`). `useResumeConversation.onSuccess` (`use-hitl.ts:79-84`) invalidates queries but never reloads messages — the manager chat panel silently never shows the continuation today. In `use-operator-chat.ts`, **poll** `readConversation` with a bounded timeout until the conversation leaves `AWAITING_HUMAN` or a new turn appears, then reconcile messages by id. A single immediate re-read races the async continuation and can return the pre-resume state — and blind appending would then duplicate the turn when the real one arrives. Do not add `resume/stream` in this PR.
...
4. **Manager: `isWriteScopeAvailable` signature change + canary scaffolding.** All preconditions wired, all still resolving `false` because `WRITE_ENDPOINTS` is empty. Tests 1–2 land here and pass *because* the list is empty. The canary is written here but cannot yet prove anything — it has no write tool to provoke.
5. **Backend: server-side resolved-request preview + fingerprint** (§3). Approval must bind to the request that will actually execute, not to a client-side reconstruction of it. This is the real precondition for granting a write.
6. **Populate `WRITE_ENDPOINTS` (4 entries) + add `GET /schedulestore/schedules` to reads, and run the canary against the real write tools.** This is the only commit that grants a write, and by this point every test above is already guarding it: a write cannot resolve to an exempt name (test 3), provisioning cannot omit the gate (test 1), the approved fingerprint is re-checked before execution (step 5), and activation cannot enable the scope without a passing canary that exercised an actual write.
