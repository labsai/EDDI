...
**Status:** this PR carries the plan and the interim documentation only — **no runtime change is in it**. Increment 1 ships in #803 (discovery) and #804 (the Keycloak client); Increment 2 in #805 (audience validation). Until those merge, EDDI advertises no OAuth metadata and does not validate the token audience, which is what `docs/mcp-server.md` describes in this PR. The optional DCR phase is not started, and the open questions below still stand.
